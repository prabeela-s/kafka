package kafka.workshop.streams;
 ...
 // from kafka streams example.
// https://raw.githubusercontent.com/confluentinc/kafka-streams-examples/5.5.1-post/src/main/java/io/confluent/examples/streams/AnomalyDetectionLambdaExample.java
 ...
 * than twice in the click stream during one minute.
 ...
 kafka-topics --create --topic UserClicks   --zookeeper localhost:2181 --partitions 1 --replication-factor 1
 kafka-topics --create --topic AnomalousUsers  --zookeeper localhost:2181 --partitions 1 --replication-factor 1
...
kafka-console-consumer --topic AnomalousUsers --from-beginning  --bootstrap-server localhost:9092  --property print.key=true  --property value.deserializer=org.apache.kafka.common.serialization.LongDeserializer

the output format is "[USER@WINDOW_START_TIME] COUNT".
...
public class AnomalyDetectionLambdaExample {
    ...
    public static void main(final String[] args) {
        ...
        //  .windowedBy(TimeWindows.of(Duration.ofMinutes(1))) means, create a table every minute, count the user within that minute
        ...
        final KTable<Windowed<String>, Long> anomalousUsers = views
                ...
                .count() // per window of every minute
                // get users whose one-minute count is >= 3
                .filter((windowedUserId, count) ->   count >= 3); // if a user checkout/click a page 3 times in a minute, flag that user
        ...
        final KStream<String, Long> anomalousUsersForConsole = anomalousUsers
                ...
                // transformation, one format to anotehr format, key-value pair
                //windowedUserId consist of key ex Alice, window() start and end time
                //create key value pair, key is (username, starttime, endtime), value is count
